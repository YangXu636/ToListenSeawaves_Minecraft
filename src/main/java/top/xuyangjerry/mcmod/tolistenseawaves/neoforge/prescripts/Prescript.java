...
public record Prescript(PrescriptDisplay display, long timeLimitTicks, MinMaxBounds.Ints cd,
                        Map<String, PrescriptCriterion<?>> criteria, boolean orderedRequirements,
                        ...
                        PrescriptPublishConditions publishConditions)
{
    ...
    private void write(RegistryFriendlyByteBuf buffer) {    //传出必要数据
        ...
        buffer.writeBoolean(this.orderedRequirements);
    ...
    private static Prescript read(RegistryFriendlyByteBuf buffer) { //按照write的顺序读取数据
        return new Prescript(PrescriptDisplay.STREAM_CODEC.decode(buffer), buffer.readLong(), MinMaxBounds.Ints.STREAM_CODEC.decode(buffer), Map.of(), buffer.readBoolean(), new PrescriptRequirements(buffer), PrescriptRewards.EMPTY, new PrescriptPublishConditions(buffer));
    }
    ...
    static {
        ...
        codec1 = RecordCodecBuilder.create((instance) -> instance.group(
                ...
                Codec.BOOL.optionalFieldOf("ordered_requirements", false).forGetter(Prescript::orderedRequirements),
